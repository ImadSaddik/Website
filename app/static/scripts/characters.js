...
document.addEventListener('DOMContentLoaded', async function() {
    modelLoaded = false;
    model = await tf.loadLayersModel('static/models/characters/model.json');
    modelLoaded = true;
...
function modelPrediction() {
    ...
    img.onload = function() {
        ...
        model.predict(tensor).data().then(predictions => {
            ...
            const classNameArray = topNValues.map(x => x.className).map(x => String.fromCharCode(x + 65));
            ...
            // The model struggled on N, O, V, F
        });
    };
    img.src = dataURL;
}
...
function changeColorOfTheHighestBar(array) {
    backgroundColorArray = new Array(26).fill(defaultBarColor); 
